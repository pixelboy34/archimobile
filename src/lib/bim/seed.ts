...
function simpleBox(id: string, name: string, city: string, w: number, d: number, lat: number, lon: number): Project {
  ...
  return {
    ...
    roofs: [{ id: uid('roof'), storyId: story.id, polygon: rectPolygon(ox - 0.3, oz - 0.3, w + 0.6, d + 0.6), ridgeHeight: 0.35, overhang: 0.3, mode: 'terrasse', pitchDeg: 30 }],
    columns: [],
    stairs: [],
    railings: [],
