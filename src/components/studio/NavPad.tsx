  ...
  return (
    <div className="nav-pad pointer-events-auto absolute top-[calc(env(safe-area-inset-top)+3.1rem)] right-2 z-10 flex flex-col items-end gap-1.5">
      {showCompass && (
        <button
          type="button"
          title="Orienter au nord"
          aria-label="Orienter au nord"
          onClick={() => dispatchCam({ kind: "north" })}
          className="relative flex size-11 items-center justify-center rounded-full bg-surface/90 shadow-border"
        >
      ...
      <div className="grid grid-cols-3 overflow-hidden bg-surface/90 shadow-border">
...
function PadBtn({
  ...
  return (
    <button
      ...
      aria-label={hint}
      onClick={onClick}
      className="flex size-10 items-center justify-center font-mono text-[10px] text-muted hover:bg-elevated hover:text-fg"
    >
      {children}
    </button>
  );
